...
function ModelSelect({ label, value, models, capabilities, onChange }: {
  label: string; value?: string; models: ModelConfigSummary[]; capabilities: ModelCapabilityKey[]; onChange: (modelId: string) => void;
}) {
...
export function WorkflowWizardV3({ initialRunId }: { initialRunId?: string | null }) {
  ...
  useEffect(() => { marketing2Api.models().then(setModels).catch(() => undefined); }, []);
  ...
  const plans = useMemo(() => {
    const result = detail?.task.stepResults?.prompt_planning?.result as { plans?: { kind: string; index: number; keyword?: string }[] } | undefined;
    return result?.plans ?? [];
  }, [detail]);

  const runAction = useCallback(async (action: () => Promise<void>) => {
    setBusy(true);
    try { await action(); } catch (error) { toast.error(error instanceof Error ? error.message : '操作失败'); } finally { setBusy(false); }
  }, []);
  ...
  const execute = (stepKey: string) => runAction(async () => {
  ...
  const statusLabel = detail?.task.status === 'completed' ? '已完成' : dirty ? '未保存' : '已保存';
  ...
  if (page === 'product_preparation') {
    ...
    content = <div className="space-y-5">
      ...
      {detail && <PromptPlanningStep detail={detail} editableAnalysis={activeStep === 'visual_analysis' && stepState(activeStep) === 'awaiting_review'} editablePlans={activeStep === 'prompt_planning' && stepState(activeStep) === 'awaiting_review'} edits={edits} onEditsChange={setEdits} />}
    </div>;
  } else if (page === 'image_generation') {
    content = <div className="space-y-5">
      {plans.length > 0 && <section className="space-y-3"><h3 className="text-sm font-medium">逐张图片模型</h3><div className="grid gap-3 md:grid-cols-2">{plans.map((plan) => {
        const key = `${plan.kind === 'main_image' ? 'main_image' : 'detail_page'}:${plan.index}`;
        return <ModelSelect key={key} label={`${plan.kind === 'main_image' ? '主图' : '详情页'} ${plan.index}${plan.keyword ? ` · ${plan.keyword}` : ''}`} value={selections.imageGeneration.items[key]} models={models} capabilities={['imageGeneration', 'referenceImage']} onChange={(modelId) => void saveSelection(`imageGeneration:${key}`, modelId)} />;
      })}</div></section>}
    ...
    content = <div className="space-y-5">
      {assets.length > 0 && <section className="space-y-3"><h3 className="text-sm font-medium">逐图质检模型</h3><div className="grid gap-3 md:grid-cols-2">{assets.map((asset) => <ModelSelect key={asset.id} label={asset.filename} value={selections.quality.items[asset.id]} models={models} capabilities={['vision', 'jsonMode']} onChange={(modelId) => void saveSelection(`quality:${asset.id}`, modelId)} />)}</div>
  ...
  return <div className="container mx-auto space-y-5 p-4 pb-28 sm:p-6 sm:pb-28">
    ...
    <div className="fixed inset-x-0 bottom-0 z-20 border-t bg-background/95 backdrop-blur"><div className="container mx-auto flex items-center justify-between gap-3 p-3 sm:px-6"><Button variant="outline" disabled={busy || page === 'product_preparation'} onClick={() => window.history.back()}>上一步</Button><Button disabled={busy || detail?.task.status === 'completed'} onClick={action.onClick}>{action.label}</Button></div></div>
  </div>;
}
